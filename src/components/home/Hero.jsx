...
const Hero = () => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', minHeight: 'calc(100vh - 70px)', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
      ...
      {/* Dark Overlay */}
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(5,15,40,0.92) 40%, rgba(5,15,40,0.5) 100%)', zIndex: 1 }} />
      ...
      <Container style={{ position: 'relative', zIndex: 2, padding: '40px 20px 60px' }}>
        <Row>
          <Col lg={8} xl={7}>
            ...
            {/* Headline */}
            <h1 style={{ color: '#ffffff', fontSize: 'clamp(1.8rem, 5vw, 3.8rem)', fontWeight: '300', lineHeight: '1.15', marginBottom: '10px', letterSpacing: '-0.5px' }}>
              {t('hero.headline1')}
            </h1>
            <h1 style={{ color: '#4a90d9', fontSize: 'clamp(2rem, 5.5vw, 4.2rem)', fontWeight: '800', lineHeight: '1.1', marginBottom: '16px', letterSpacing: '-1px' }}>
              {t('hero.headline2')}
            </h1>

            {/* Subtitle */}
            <p style={{ color: '#b0c4de', fontSize: 'clamp(0.88rem, 1.5vw, 1.1rem)', lineHeight: '1.7', marginBottom: '24px', maxWidth: '520px' }}>
              {t('hero.subtitle')}
            </p>

            {/* Search Box */}
            <div style={{ backgroundColor: 'rgba(255,255,255,0.97)', borderRadius: '12px', padding: '6px', maxWidth: '600px', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', marginBottom: '36px' }}>
              {/* Tabs */}
              <div style={{ display: 'flex', gap: '4px', padding: '4px 4px 10px 4px', borderBottom: '1px solid #e8ecf2', overflowX: 'auto' }}>
                {tabs.map(tab => (
                  <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                    style={{ padding: '7px 14px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '0.8rem', fontWeight: '600', transition: 'all 0.2s ease', backgroundColor: activeTab === tab.key ? '#0d1f4e' : 'transparent', color: activeTab === tab.key ? '#ffffff' : '#666', whiteSpace: 'nowrap', flexShrink: 0 }}>
                    {tab.label}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 8px 4px 8px' }}>
                ...
                <input
                  ...
                  style={{ flex: 1, border: 'none', outline: 'none', fontSize: '0.9rem', color: '#1a1a2e', backgroundColor: 'transparent', padding: '8px 4px', minWidth: 0 }}
                />
                <button onClick={handleSearch}
                  style={{ backgroundColor: '#2d5fc4', color: '#ffffff', border: 'none', borderRadius: '8px', padding: '12px 20px', fontSize: '0.88rem', fontWeight: '600', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0, transition: 'background 0.2s ease' }}
            ...
            <div style={{ display: 'flex', gap: '0', flexWrap: 'wrap' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  <div>
                    <div style={{ color: '#ffffff', fontSize: 'clamp(1.4rem, 3vw, 2rem)', fontWeight: '700', lineHeight: '1' }}>
                      <CounterNumber target={stat.target} suffix={stat.suffix} duration={2000} />
                    </div>
                    <div style={{ color: '#8aafd4', fontSize: '0.78rem', letterSpacing: '0.5px', marginTop: '4px' }}>{stat.label}</div>
                  </div>
                </div>
              ))}
            </div>
