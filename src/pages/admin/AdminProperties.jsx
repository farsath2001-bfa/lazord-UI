    ...
    <div style={{ backgroundColor: '#060f26', minHeight: '100vh' }}>
      ...
      <div className="ap-container">
        ...
        <div className="ap-header">
          <div>
            <h1 style={{ color: '#ffffff', fontSize: 'clamp(1.3rem, 4vw, 1.8rem)', fontWeight: '700', margin: 0 }}>Admin Panel</h1>
            <p style={{ color: '#8aafd4', margin: '4px 0 0', fontSize: '0.82rem' }}>
          ...
          {activeTab === 'properties' && (
            <button onClick={() => { setShowForm(!showForm); setForm(emptyForm); setEditId(null) }}
              style={{ backgroundColor: showForm ? 'rgba(231,76,60,0.2)' : '#2d5fc4', color: showForm ? '#e74c3c' : '#fff', border: showForm ? '1px solid rgba(231,76,60,0.4)' : 'none', borderRadius: '10px', padding: '10px 18px', fontSize: '0.88rem', fontWeight: '700', cursor: 'pointer', whiteSpace: 'nowrap' }}>
              {showForm ? '✕ Cancel' : '+ Add Property'}
            </button>
          )}
          {activeTab === 'enquiries' && (
            <button onClick={fetchEnquiries}
              style={{ backgroundColor: 'rgba(45,95,196,0.2)', color: '#4a90d9', border: '1px solid rgba(45,95,196,0.3)', borderRadius: '10px', padding: '10px 18px', fontSize: '0.88rem', fontWeight: '700', cursor: 'pointer' }}>
              🔄 Refresh
            </button>
          )}
        </div>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', backgroundColor: '#0d1f4e', borderRadius: '12px', padding: '6px', width: 'fit-content', border: '1px solid rgba(45,95,196,0.25)' }}>
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              style={{ backgroundColor: activeTab === tab.key ? '#2d5fc4' : 'transparent', color: activeTab === tab.key ? '#ffffff' : '#8aafd4', border: 'none', borderRadius: '8px', padding: '8px 16px', fontSize: '0.85rem', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '6px' }}>
              {tab.label}
              {tab.count > 0 && (
                <span style={{ backgroundColor: tab.badge && activeTab !== tab.key ? '#e74c3c' : 'rgba(255,255,255,0.2)', color: '#fff', borderRadius: '20px', padding: '1px 8px', fontSize: '0.7rem', fontWeight: '800' }}>{tab.count}</span>
              )}
            </button>
          ))}
        </div>
        ...
        {/* Message */}
        {message && (
          <div style={{ backgroundColor: message.includes('✅') ? 'rgba(39,174,96,0.15)' : 'rgba(231,76,60,0.15)', border: `1px solid ${message.includes('✅') ? 'rgba(39,174,96,0.3)' : 'rgba(231,76,60,0.3)'}`, borderRadius: '10px', padding: '12px 16px', color: message.includes('✅') ? '#27ae60' : '#e74c3c', marginBottom: '16px', fontWeight: '600', fontSize: '0.88rem' }}>
            {message}
          </div>
        )}
        ...
        {activeTab === 'properties' && (
          <>
            {/* Form */}
            {showForm && (
              <div style={{ backgroundColor: '#0d1f4e', border: '1px solid rgba(45,95,196,0.3)', borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
                <h2 style={{ color: '#ffffff', fontSize: '1.1rem', fontWeight: '700', marginBottom: '20px' }}>
                ...
                <form onSubmit={handleSubmit}>
                  <div className="ap-form-grid">
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                      ...
                      {form.image && <img src={form.image} alt="preview" style={{ width: '100px', height: '75px', objectFit: 'cover', borderRadius: '8px', marginTop: '10px' }} />}
                    </div>
                    <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <button type="submit" disabled={saving}
                    style={{ marginTop: '20px', backgroundColor: saving ? '#1a3a7c' : '#2d5fc4', color: '#fff', border: 'none', borderRadius: '10px', padding: '12px 28px', fontSize: '0.9rem', fontWeight: '700', cursor: saving ? 'not-allowed' : 'pointer', width: '100%' }}>
            ...
            <div style={{ backgroundColor: '#0d1f4e', border: '1px solid rgba(45,95,196,0.25)', borderRadius: '16px', overflow: 'hidden' }}>
              <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(45,95,196,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                <h2 style={{ color: '#ffffff', fontSize: '0.95rem', fontWeight: '700', margin: 0 }}>
                ...
                <>
                  ...
                  <div className="ap-cards-mobile">
                    {filteredProperties.map((p) => (
                      <div key={p._id} style={{ padding: '14px 16px', borderBottom: '1px solid rgba(45,95,196,0.1)' }}>
                        <div style={{ display: 'flex', gap: '12px', marginBottom: '10px' }}>
                          <img src={p.image} alt={p.title} style={{ width: '70px', height: '55px', objectFit: 'cover', borderRadius: '8px', flexShrink: 0 }} />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ color: '#ffffff', fontSize: '0.88rem', fontWeight: '700', marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</div>
                            <div style={{ color: '#8aafd4', fontSize: '0.72rem', marginBottom: '4px' }}>{p.category} · {p.bedrooms === 0 ? 'Studio' : `${p.bedrooms} BR`}</div>
                            <div style={{ color: '#ffffff', fontSize: '0.85rem', fontWeight: '700' }}>AED {p.price?.toLocaleString()}</div>
                          </div>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                            <span style={{ backgroundColor: p.type === 'Buy' ? 'rgba(45,95,196,0.2)' : p.type === 'Rent' ? 'rgba(39,174,96,0.2)' : 'rgba(142,68,173,0.2)', color: p.type === 'Buy' ? '#4a90d9' : p.type === 'Rent' ? '#27ae60' : '#8e44ad', padding: '2px 8px', borderRadius: '20px', fontSize: '0.7rem', fontWeight: '600' }}>{p.type}</span>
                            <span style={{ backgroundColor: p.status === 'Available' ? 'rgba(39,174,96,0.2)' : 'rgba(231,76,60,0.2)', color: p.status === 'Available' ? '#27ae60' : '#e74c3c', padding: '2px 8px', borderRadius: '20px', fontSize: '0.7rem', fontWeight: '600' }}>{p.status}</span>
                          </div>
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button onClick={() => handleEdit(p)} style={{ backgroundColor: 'rgba(74,144,217,0.15)', color: '#4a90d9', border: '1px solid rgba(74,144,217,0.3)', borderRadius: '6px', padding: '5px 10px', fontSize: '0.75rem', fontWeight: '600', cursor: 'pointer' }}>✏️</button>
                            <PropertyPDFButton property={p} />
                            <button onClick={() => handleDelete(p._id)} style={{ backgroundColor: 'rgba(231,76,60,0.15)', color: '#e74c3c', border: '1px solid rgba(231,76,60,0.3)', borderRadius: '6px', padding: '5px 10px', fontSize: '0.75rem', cursor: 'pointer' }}>🗑️</button>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  {/* Desktop Table */}
                  <div className="ap-table-desktop" style={{ overflowX: 'auto' }}>
                          ...
                          <tr key={p._id} style={{ borderTop: '1px solid rgba(45,95,196,0.15)', backgroundColor: i % 2 === 0 ? 'transparent' : 'rgba(45,95,196,0.04)', transition: 'background 0.15s' }}
                            ...
                            <td style={{ padding: '12px 16px' }}>
                              <div style={{ display: 'flex', gap: '6px' }}>
                                ...
                                <PropertyPDFButton property={p} />
                                <button onClick={() => handleDelete(p._id)} style={{ backgroundColor: 'rgba(231,76,60,0.15)', color: '#e74c3c', border: '1px solid rgba(231,76,60,0.3)', borderRadius: '6px', padding: '6px 12px', fontSize: '0.78rem', fontWeight: '600', cursor: 'pointer' }}>🗑️</button>
                              </div>
                            </td>
                          </tr>
                        ))}
        ...
        {/* ENQUIRIES TAB */}
        {activeTab === 'enquiries' && (
          <div className={`ap-enquiries-grid ${selectedEnquiry ? 'ap-enquiries-open' : ''}`}>
            <div style={{ backgroundColor: '#0d1f4e', border: '1px solid rgba(45,95,196,0.25)', borderRadius: '16px', overflow: 'hidden' }}>
              <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(45,95,196,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                <h2 style={{ color: '#ffffff', fontSize: '0.95rem', fontWeight: '700', margin: 0 }}>All Enquiries ({enquiries.length})</h2>
                {newEnquiriesCount > 0 && (
              ...
              {enquiriesLoading ? (
                ...
                <>
                  ...
                  <div className="ap-cards-mobile">
                    {enquiries.map((e) => (
                      <div key={e._id}
                        onClick={() => setSelectedEnquiry(selectedEnquiry?._id === e._id ? null : e)}
                        style={{ padding: '14px 16px', borderBottom: '1px solid rgba(45,95,196,0.1)', cursor: 'pointer', backgroundColor: selectedEnquiry?._id === e._id ? 'rgba(45,95,196,0.15)' : 'transparent' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <div style={{ width: '34px', height: '34px', borderRadius: '50%', backgroundColor: 'rgba(45,95,196,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#4a90d9', fontWeight: '800', fontSize: '0.85rem', flexShrink: 0 }}>
                              {e.name?.charAt(0).toUpperCase()}
                            </div>
                            <div>
                              <div style={{ color: '#ffffff', fontWeight: '700', fontSize: '0.88rem' }}>{e.name}</div>
                              <div style={{ color: '#8aafd4', fontSize: '0.75rem' }}>{e.phone}</div>
                            </div>
                          </div>
                          <select value={e.status} onChange={ev => { ev.stopPropagation(); handleEnquiryStatusChange(e._id, ev.target.value) }}
                            onClick={ev => ev.stopPropagation()}
                            style={{ backgroundColor: statusColor(e.status).bg, color: statusColor(e.status).color, border: `1px solid ${statusColor(e.status).color}40`, borderRadius: '20px', padding: '3px 8px', fontSize: '0.7rem', fontWeight: '700', cursor: 'pointer', outline: 'none' }}>
                            {['New', 'Contacted', 'In Progress', 'Closed', 'Lost'].map(s => <option key={s} value={s}>{s}</option>)}
                          </select>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ backgroundColor: 'rgba(45,95,196,0.15)', color: '#4a90d9', padding: '2px 8px', borderRadius: '20px', fontSize: '0.68rem', fontWeight: '600' }}>{e.source}</span>
                          <button onClick={ev => { ev.stopPropagation(); handleEnquiryDelete(e._id) }}
                            style={{ backgroundColor: 'rgba(231,76,60,0.15)', color: '#e74c3c', border: '1px solid rgba(231,76,60,0.3)', borderRadius: '6px', padding: '4px 8px', fontSize: '0.75rem', cursor: 'pointer' }}>🗑️</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="ap-detail-panel">
                ...
                {selectedEnquiry.message && (
                  <div style={{ marginBottom: '20px' }}>
                    <div style={{ color: '#8aafd4', fontSize: '0.7rem', textTransform: 'uppercase', marginBottom: '6px' }}>💬 Message</div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <div style={{ color: '#8aafd4', fontSize: '0.7rem', textTransform: 'uppercase', marginBottom: '6px' }}>📊 Update Status</div>
      ...
      <style>{`
        .ap-container { padding: 16px 14px; }
        .ap-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px; flex-wrap: wrap; gap: 10px; }
        .ap-form-grid { display: grid; grid-template-columns: 1fr; gap: 14px; }
        .ap-cards-mobile { display: block; }
        .ap-table-desktop { display: none; }
        .ap-enquiries-grid { display: grid; grid-template-columns: 1fr; gap: 16px; }
        .ap-detail-panel { background-color: #0d1f4e; border: 1px solid rgba(45,95,196,0.3); border-radius: 16px; padding: 20px; }

        @media (min-width: 640px) {
          .ap-container { padding: 20px 18px; }
          .ap-form-grid { grid-template-columns: repeat(2, 1fr); }
        }

        @media (min-width: 1024px) {
          .ap-container { padding: 32px 30px; }
          .ap-form-grid { grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); }
          .ap-cards-mobile { display: none; }
          .ap-table-desktop { display: block; }
          .ap-enquiries-grid.ap-enquiries-open { grid-template-columns: 1fr 380px; }
          .ap-detail-panel { position: sticky; top: 80px; }
        }
      `}</style>
    </div>
  )
