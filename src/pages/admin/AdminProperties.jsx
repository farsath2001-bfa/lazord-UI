  ...
  return (
    <div style={{ backgroundColor: '#060f26', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '40px 30px' }}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: '4px', marginBottom: '28px', backgroundColor: '#0d1f4e', borderRadius: '12px', padding: '6px', width: 'fit-content', border: '1px solid rgba(45,95,196,0.25)' }}>
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              ...
              {tab.count > 0 && (
                <span style={{
                  backgroundColor: tab.badge && activeTab !== tab.key ? '#e74c3c' : 'rgba(255,255,255,0.2)',
                  color: '#fff', borderRadius: '20px', padding: '1px 8px', fontSize: '0.72rem', fontWeight: '800'
                }}>{tab.count}</span>
              )}
            </button>
          ))}
          ...
          <>
            ...
            <div style={{ backgroundColor: '#0d1f4e', border: '1px solid rgba(45,95,196,0.25)', borderRadius: '16px', overflow: 'hidden' }}>
              <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(45,95,196,0.2)' }}>
                <h2 style={{ color: '#ffffff', fontSize: '1.1rem', fontWeight: '700', margin: 0 }}>All Properties ({properties.length})</h2>
              </div>
              {loading ? (
                    ...
                    <tbody>
                      {properties.map((p, i) => (
                        <tr key={p._id} style={{ borderTop: '1px solid rgba(45,95,196,0.15)', backgroundColor: i % 2 === 0 ? 'transparent' : 'rgba(45,95,196,0.04)' }}>
                          ...
                          <td style={{ padding: '12px 16px', color: '#ffffff', fontSize: '0.85rem', fontWeight: '700', whiteSpace: 'nowrap' }}>AED {p.price?.toLocaleString()}</td>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: selectedEnquiry ? '1fr 380px' : '1fr', gap: '20px', alignItems: 'start' }}>
            ...
            <div style={{ backgroundColor: '#0d1f4e', border: '1px solid rgba(45,95,196,0.25)', borderRadius: '16px', overflow: 'hidden' }}>
              <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(45,95,196,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h2 style={{ color: '#ffffff', fontSize: '1.1rem', fontWeight: '700', margin: 0 }}>All Enquiries ({enquiries.length})</h2>
                {newEnquiriesCount > 0 && (
              ...
              {enquiriesLoading ? (
                <div style={{ padding: '40px', textAlign: 'center', color: '#8aafd4' }}>Loading enquiries...</div>
              ) : enquiries.length === 0 ? (
                    ...
                    <tbody>
                      {enquiries.map((e, i) => (
                        <tr key={e._id}
                          ...
                          style={{ borderTop: '1px solid rgba(45,95,196,0.15)', backgroundColor: selectedEnquiry?._id === e._id ? 'rgba(45,95,196,0.12)' : i % 2 === 0 ? 'transparent' : 'rgba(45,95,196,0.04)', cursor: 'pointer', transition: 'background 0.15s' }}>
                          <td style={{ padding: '12px 16px' }}>
                            <div style={{ color: '#ffffff', fontWeight: '700', fontSize: '0.85rem' }}>{e.name}</div>
                            {e.status === 'New' && <div style={{ color: '#e74c3c', fontSize: '0.7rem', fontWeight: '700', marginTop: '2px' }}>● NEW</div>}
                          </td>
                          <td style={{ padding: '12px 16px' }}>
                            <div style={{ color: '#8aafd4', fontSize: '0.82rem' }}>{e.email}</div>
                            <div style={{ color: '#8aafd4', fontSize: '0.82rem' }}>{e.phone}</div>
                          </td>
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ backgroundColor: 'rgba(45,95,196,0.15)', color: '#4a90d9', padding: '3px 10px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: '600' }}>{e.source}</span>
                          </td>
                          <td style={{ padding: '12px 16px', color: '#8aafd4', fontSize: '0.82rem', maxWidth: '160px' }}>
                            {e.property?.title || <span style={{ color: '#4a4a6a' }}>—</span>}
                          </td>
                          <td style={{ padding: '12px 16px' }} onClick={ev => ev.stopPropagation()}>
                            <select
                              value={e.status}
                              onChange={ev => handleEnquiryStatusChange(e._id, ev.target.value)}
                          ...
                          <td style={{ padding: '12px 16px', color: '#8aafd4', fontSize: '0.8rem', whiteSpace: 'nowrap' }}>
                          ...
                          <td style={{ padding: '12px 16px' }} onClick={ev => ev.stopPropagation()}>
                            <button onClick={() => handleEnquiryDelete(e._id)}
                              style={{ backgroundColor: 'rgba(231,76,60,0.15)', color: '#e74c3c', border: '1px solid rgba(231,76,60,0.3)', borderRadius: '6px', padding: '6px 12px', fontSize: '0.78rem', fontWeight: '600', cursor: 'pointer' }}>
                              🗑️
                            </button>
                          </td>
                        </tr>
            ...
            {/* Detail Panel */}
            {selectedEnquiry && (
              <div style={{ backgroundColor: '#0d1f4e', border: '1px solid rgba(45,95,196,0.3)', borderRadius: '16px', padding: '24px', position: 'sticky', top: '100px' }}>
                ...
                <div style={{ width: '56px', height: '56px', borderRadius: '50%', backgroundColor: 'rgba(45,95,196,0.3)', border: '2px solid rgba(74,144,217,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', fontWeight: '700', color: '#4a90d9', marginBottom: '16px' }}>
                  {selectedEnquiry.name?.charAt(0).toUpperCase()}
                </div>

                <div style={{ color: '#ffffff', fontWeight: '700', fontSize: '1.1rem', marginBottom: '4px' }}>{selectedEnquiry.name}</div>
                <div style={{ color: '#8aafd4', fontSize: '0.85rem', marginBottom: '20px' }}>
                ...
                {[
                  ...
                  <div key={row.label} style={{ marginBottom: '14px' }}>
                    <div style={{ color: '#8aafd4', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>{row.icon} {row.label}</div>
                ...
                {selectedEnquiry.message && (
                  <div style={{ marginBottom: '20px' }}>
                    <div style={{ color: '#8aafd4', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '6px' }}>💬 Message</div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <div style={{ color: '#8aafd4', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '6px' }}>📊 Update Status</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <a href={`mailto:${selectedEnquiry.email}`}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', backgroundColor: 'rgba(45,95,196,0.2)', color: '#4a90d9', border: '1px solid rgba(45,95,196,0.3)', borderRadius: '8px', padding: '10px', textDecoration: 'none', fontWeight: '600', fontSize: '0.85rem' }}>
                    📧 Send Email
                  </a>
                  <a href={`https://wa.me/${selectedEnquiry.phone?.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', backgroundColor: 'rgba(37,211,102,0.15)', color: '#25d366', border: '1px solid rgba(37,211,102,0.3)', borderRadius: '8px', padding: '10px', textDecoration: 'none', fontWeight: '600', fontSize: '0.85rem' }}>
                    💬 WhatsApp
                  </a>
                  <a href={`tel:${selectedEnquiry.phone}`}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', backgroundColor: 'rgba(39,174,96,0.15)', color: '#27ae60', border: '1px solid rgba(39,174,96,0.3)', borderRadius: '8px', padding: '10px', textDecoration: 'none', fontWeight: '600', fontSize: '0.85rem' }}>
                    📞 Call Now
                  </a>
                </div>
              </div>
